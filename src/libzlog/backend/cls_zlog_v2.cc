...
static int __max_position_v2(cls_method_context_t hctx, uint64_t *pposition)
{
  ...
  zlog_ceph_proto::MaxPositionRet content;
...
static int read_v2(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  zlog_ceph_proto::LogEntryV2 entry;
  ...
  if (entry.flags() & zlog_ceph_proto::LogEntryV2::INVALIDATED ||
      entry.flags() & zlog_ceph_proto::LogEntryV2::TRIMMED)
...
static int fill_v2(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  if (ret == -ENOENT) {
    entry.set_flags(static_cast<zlog_ceph_proto::LogEntryV2_Flags>(zlog_ceph_proto::LogEntryV2::DEFAULT| zlog_ceph_proto::LogEntryV2::INVALIDATED));
    entry.set_flags(static_cast<zlog_ceph_proto::LogEntryV2_Flags>(zlog_ceph_proto::LogEntryV2::DEFAULT| zlog_ceph_proto::LogEntryV2::TRIMMED));
    bufferlist entrybl;
    encode(entrybl, entry);
    ret = cls_cxx_map_set_val(hctx, key, &entrybl);
    if (ret < 0) {
...
static int trim_v2(cls_method_context_t hctx, bufferlist *in, bufferlist *out)
{
  ...
  if (entry.flags() & zlog_ceph_proto::LogEntryV2::TRIMMED)
    return CLS_ZLOG_OK;
