...
#include <cstring>       // For std::vector
...
public:
    ...
    // Number of chars in the string (excluding the terminating NUL).
    ...
    int Compare(const String& other) const noexcept
    {
        ...
        const int result = memcmp(m_ptr, other.m_ptr, minLength);
...
private:
    const char* m_ptr{};     // C-style raw pointer to a NUL-terminated string
    size_t m_length{};          // Length, in chars, excluding the terminating NUL
    ...
    String(const char* ptr, size_t length) noexcept
        : m_ptr{ ptr }
        , m_length{ length }
    {}
};
...
public:
    ...
    String AllocString(const char* ptr)
    {
    ...
    // Throws std::bad_alloc on allocation failure.
    String AllocString(const char* start, const char* finish)
    {
